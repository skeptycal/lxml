...
def ext_modules(
    ...
    _include_dirs = _prefer_reldirs(
        ...
        + [SOURCE_PATH, INCLUDE_PACKAGE_PATH,],
    )
...
def define_macros():
    ...
    macros.append(
        ("CYTHON_CLINE_IN_TRACEBACK", "1" if OPTION_WITH_CLINES else "0")
    )
    return macros
...
def check_min_version(version, min_version, libname):
    ...
    if lib_version < req_version:
        print(
            "Minimum required version of %s is %s. Your system has version %s."
...
def check_build_dependencies():
    ...
    if xml2_version and xslt_version:
        print(
            "Building against libxml2 %s and libxslt %s"
            % (xml2_version, xslt_version)
        )
    else:
