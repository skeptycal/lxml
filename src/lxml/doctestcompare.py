...
class LXMLOutputChecker(OutputChecker):
    ...
    def get_parser(self, want, got, optionflags):
        ...
        if PARSE_HTML & optionflags:
            ...
            parser = etree.XML
        elif want.strip().lower().startswith(
            "<html"
        ...
        if "any" not in want.attrib:
            ...
            for key in want_keys:
                if not self.text_compare(
                    want.attrib[key], got.attrib[key], False
                ):
                    return False
    ...
    def tag_compare(self, want, got):
        if want == "any":
            return True
        if not isinstance(want, _basestring) or not isinstance(
            got, _basestring
        ):
            return want == got
        want = want or ""
    ...
    def collect_diff(self, want, got, html, indent):
        ...
        while want_children or got_children:
            ...
            if not got_children:
                parts.append(
                    self.format_doc(
                        want_children.pop(0), html, indent + 2, "-"
                    )
                )
                continue
            parts.append(
        ...
        for name, value in sorted(got.attrib.items()):
            if name not in want.attrib and not any:
                ...
                if name in want.attrib:
                    text = self.collect_diff_text(
                        want.attrib[name], value, False
                    )
                else:
