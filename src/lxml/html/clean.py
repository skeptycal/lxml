...
class Cleaner(object):
    ...
    def __call__(self, doc):
        ...
        if self.javascript:
            if not (
                self.safe_attrs_only and self.safe_attrs == defs.safe_attrs
            ):
                ...
                for el in doc.iter(etree.Element):
                    ...
                    for aname in attrib.keys():
                        if aname.startswith("on"):
                            del attrib[aname]
            doc.rewrite_links(
                self._remove_javascript_link, resolve_base_href=False
            )
        ...
        if self.add_nofollow:
            for el in _find_external_links(doc):
                if not self.allow_follow(el):
                    rel = el.get("rel")
                    if rel:
                        if "nofollow" in rel and " nofollow " in (
                            " %s " % rel
                        ):
                            continue
                        rel = "%s nofollow" % rel
    ...
    def allow_embedded_url(self, el, url):
        ...
        """
        if (
            self.whitelist_tags is not None
    ...
    def kill_conditional_comments(self, doc):
        ...
        self._kill_elements(
            doc,
            lambda el: _conditional_comment_re.search(el.text),
            etree.Comment,
        )
...
_link_regexes = [
    ...
    re.compile(
        r"mailto:(?P<body>[a-z0-9._-]+@(?P<host>[a-z0-9_.-]+[a-z]))", re.I
    ),
]
