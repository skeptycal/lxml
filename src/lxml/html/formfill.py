...
_label_for_xpath = XPath(
    "//label[@for=$for_id]|//x:label[@for=$for_id]",
    namespaces={"x": XHTML_NAMESPACE},
)
...
def fill_form(
    el, values, form_id=None, form_index=None,
):
...
class DefaultErrorCreator(object):
    ...
    def __call__(self, el, is_block, message):
        ...
        if is_block and self.error_block_class:
            error_el.set(
                "class",
                error_el.get("class", "") + " " + self.error_block_class,
            )
...
def insert_errors(
    ...
    for name, error in errors.items():
        ...
        for error_el, message in _find_elements_for_name(el, name, error):
            assert isinstance(
                message, (basestring, type(None), ElementBase)
