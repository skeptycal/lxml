...
try:
    ...
    class XHTMLParser(_XHTMLParser):
        ...
        def __init__(self, strict=False, **kwargs):
            _XHTMLParser.__init__(
                self, strict=strict, tree=TreeBuilder, **kwargs
            )
...
def fragments_fromstring(
    html, no_leading_text=False, guess_charset=None, parser=None
):
        ...
        if no_leading_text:
            if children[0].strip():
                raise etree.ParserError(
                    "There is leading text: %r" % children[0]
                )
            del children[0]
    return children
...
def fragment_fromstring(
    html, create_parent=False, guess_charset=None, parser=None
):
...
def _looks_like_url(str):
    ...
    if not scheme:
        ...
        sys.platform == "win32"
        and scheme in string.ascii_letters
        and len(scheme) == 1
    ):
