...
class ETreeDtdTestCase(HelperTestCase):
    ...
    def test_dtd_parse_invalid(self):
        ...
        xml = _bytes(
            '<!DOCTYPE b SYSTEM "%s"><b><a/></b>' % fileInTestDir("test.dtd")
        )
    ...
    def test_dtd_parse_valid(self):
        parser = etree.XMLParser(dtd_validation=True)
        xml = '<!DOCTYPE a SYSTEM "%s"><a><b/></a>' % fileUrlInTestDir(
            "test.dtd"
        )
    ...
    def test_dtd_invalid_duplicate_id(self):
        ...
        self.assertTrue(
            [error for error in dtd.error_log if "id1" in error.message]
        )
    ...
    def test_internal_dtds(self):
        for el_count in range(2, 5):
            for attr_count in range(4):
                ...
                """
                        + "<!ELEMENT el0 (%s)>"
                        % "|".join(["el%d" % e for e in range(1, el_count)])
                        + """
                ]>
                <el0><el1 %s /></el0>
                """
                        % " ".join(
                            ['attr%d="x"' % a for a in range(attr_count)]
                        )
                    )
                )
                dtd = etree.ElementTree(root).docinfo.internalDTD
    ...
    def test_dtd_broken(self):
        self.assertRaises(
            etree.DTDParseError, etree.DTD, BytesIO("<!ELEMENT b HONKEY>")
        )
    ...
    def test_iterparse_file_dtd_start(self):
        ...
        self.assertEqual(["valueA", "valueB"], attributes)
    ...
    def test_iterparse_file_dtd_end(self):
        iterparse = etree.iterparse
        iterator = iterparse(
            fileInTestDir("test.xml"), events=("end",), attribute_defaults=True
        )
        attributes = [element.get("default") for event, element in iterator]
    ...
    def test_declaration_escape_quote_pid(self):
        ...
        self.assertEqual(
            doc.docinfo.doctype, """<!DOCTYPE a PUBLIC "foo" '"'>"""
        )
        self.assertEqual(
            etree.tostring(doc),
    ...
    def test_declaration_quote_withoutpid(self):
        ...
        self.assertEqual(
            etree.tostring(doc), _bytes("""<!DOCTYPE a SYSTEM '"'>\n<a/>""")
        )
    ...
    def test_set_decl_public(self):
        ...
        self.assertEqual(
            doc.docinfo.doctype, '<!DOCTYPE test PUBLIC "bar" "baz">'
        )
        self.assertEqual(
            etree.tostring(doc),
    ...
    def test_html_decl(self):
        ...
        doc.docinfo.system_url = "baz"
        self.assertEqual(
            doc.docinfo.doctype, '<!DOCTYPE html PUBLIC "bar" "baz">'
        )
        self.assertEqual(
            etree.tostring(doc),
    ...
    def test_empty_decl(self):
        ...
        self.assertEqual(
            etree.tostring(doc), _bytes("<!DOCTYPE test>\n<test/>")
        )
