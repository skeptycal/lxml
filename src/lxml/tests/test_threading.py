...
class ThreadingTestCase(HelperTestCase):
    ...
    def _run_threads(self, count, func, main_func=None):
        ...
        threads = [
            threading.Thread(target=sync_start, args=(func,))
...
""",
    ...
    <xsl:stylesheet version="1.0"
      ...
      <xsl:template match="*">
        ...
        self._run_thread(run_thread)
        self.assertEqual(
            _bytes("<a><b>B</b><c>C</c><foo><a>B</a></foo></a>"),
            tostring(root),
        )
...
"""
            + "\n".join(
                '<xsl:template match="tag%x" />' % i for i in range(200)
            )
            + """
    <xsl:UnExpectedElement />
        ...
        def run_thread():
            try:
                ...
                self.assertFalse(
                    True, "XSLT parsing should have failed but didn't"
                )
        ...
        for log in error_logs:
            ...
            self.assertTrue(
                any("UnExpectedElement" in str(error) for error in log), log
            )
            last_log = log
...
"""
            + "\n".join(
                '<xsl:template match="tag%X" name="tag%x"> <xsl:call-template name="tag%x" /> </xsl:template>'
                % (i, i, i - 1)
    ...
    <xsl:stylesheet version="1.0"
      ...
      <xsl:template match="*">
        ...
        self._run_thread(run_thread)
        self.assertEqual(_bytes('<root class="xyz"/>'), tostring(result[0]))
    ...
    <xsl:stylesheet
      ...
      <xsl:template match="/">
        ...
        self.assertEqual(_bytes('<div id="test">BC</div>'), result)
    ...
    def test_thread_error_log(self):
        ...
        for thread_no in range(1, 10):
            t = threading.Thread(target=parse_error_test, args=(thread_no,))
            threads.append(t)
            t.start()
    ...
    def test_thread_mix(self):
        ...
        def run_build():
            result.append(Element("{myns}foo", attrib={"{test}attr": "val"}))
    ...
    <xsl:stylesheet version="1.0"
      ...
      <xsl:template match="*">
        ...
        self.assertEqual(
            _bytes(
                '<ns0:root xmlns:ns0="myns" att="someval"><b>B</b>'
            ...
            for _ in range(3):
                ...
                for name in names:
                    ...
                    for _ in runs_per_name:
                        el = SubElement(
                            root, tag_name, {"thread_attr_" + name: "value"}
                        )
                        new.append(el)
                    for el in new:
    ...
    def test_concurrent_class_lookup(self):
        ...
        root = XML(
            _bytes('<root><a>A</a><b xmlns="test">B</b><c/></root>'), parser
        )
