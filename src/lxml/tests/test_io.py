...
class _IOTestCaseBase(HelperTestCase):
    ...
    def test_write_filename_special_percent(self):
        ...
        def difference(filenames):
            return sorted(
                fn
                for fn in set(filenames).difference(before_test)
    ...
    def test_write_invalid_filename(self):
        filename = os.path.join(
            os.path.join("hopefullynonexistingpathname"), "invalid_file.xml"
        )
        try:
            self.tree.write(filename)
        except IOError:
    ...
    def test_module_parse_gzipobject(self):
        ...
        with tmpfile(suffix=".xml.gz") as filename:
            ...
            self.assertEqual(
                self.etree.tostring(tree.getroot()), self.root_str
            )
    ...
    def test_etree_parse_io_error(self):
        ...
        try:
            self.assertRaises(
                IOError, self.etree.parse, os.path.join(dn, filename)
            )
        finally:
            os.rmdir(dn)
