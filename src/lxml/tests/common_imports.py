...
IS_PYPY = (
    getattr(sys, "implementation", None) == "pypy"
...
if sys.version_info[0] >= 3:
    ...
    def _str(s, encoding="UTF-8"):
        return unichr_escape.sub(
            lambda x: unicode_escape_decode(x.group(0))[0], s
        )
    ...
    def _str(s, encoding="UTF-8"):
        ...
        return unichr_escape.sub(
            lambda x: x.group(0).decode("unicode-escape"), s
        )
    ...
    doctest_parser = doctest.DocTestParser()
    _fix_traceback = re.compile(
        r"^(\s*)(?:\w+\.)+(\w*(?:Error|Exception|Invalid):)", re.M
    ).sub
...
def canonicalize(xml):
    tree = etree.parse(
        BytesIO(xml) if isinstance(xml, bytes) else StringIO(xml)
    )
    f = BytesIO()
    tree.write_c14n(f)
    return f.getvalue()
