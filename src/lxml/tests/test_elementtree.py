...
if cElementTree is not None and (
    CET_VERSION <= (1, 0, 7) or sys.version_info[0] >= 3
):
    cElementTree = None

if ElementTree is not None:
    print(
        "Comparing with ElementTree %s" % getattr(ElementTree, "VERSION", "?")
    )

if cElementTree is not None:
    print(
        "Comparing with cElementTree %s"
        % getattr(cElementTree, "VERSION", "?")
    )
        ...
        def testfunc(self, *args):
            if self.etree is not etree and sys.version_info < version:
                raise unittest.SkipTest(
                    "requires ET in Python %s" % ".".join(map(str, version))
                )
...
class _ETreeTestCaseBase(HelperTestCase):
    ...
    def test_interface(self):
        ...
        def check_element(element):
            self.assertTrue(
                self.etree.iselement(element), msg="not an element"
            )
            direlem = dir(element)
            for attr in "tag", "attrib", "text", "tail":
                self.assertTrue(
                    hasattr(element, attr), msg="no %s member" % attr
                )
        ...
        def check_method(method):
            self.assertTrue(
                hasattr(method, "__call__"), msg="%s not callable" % method
            )
    ...
    def test_text_escape_tostring(self):
        ...
        a = Element("a")
        a.text = "<>&"
        self.assertEqual(_bytes("<a>&lt;&gt;&amp;</a>"), tostring(a))
    ...
    def test_text_str_subclass(self):
        ...
        a = Element("a")
        a.text = strTest("text")
        self.assertXML(_bytes("<a>text</a>"), a)
    ...
    def test_tail_str_subclass(self):
        ...
        self.assertXML(_bytes("<a><t></t>tail</a>"), a)
    ...
    def test_attribute_update_dict(self):
        ...
        items.sort()
        self.assertEqual([("alpha", "Alpha"), ("beta", "Beta")], items)
        ...
        items.sort()
        self.assertEqual(
            [("alpha", "test"), ("beta", "Beta"), ("gamma", "Gamma")], items
        )
    ...
    def test_attribute_items2(self):
        ...
        items.sort()
        self.assertEqual(
            [("alpha", "Alpha"), ("beta", "Beta"), ("gamma", "Gamma")], items
        )
    ...
    def test_attribute_keys_ns(self):
        ...
        keys = list(root.keys())
        keys.sort()
        self.assertEqual(["bar", "{http://ns.codespeak.net/test}baz"], keys)
    ...
    def test_attribute_values_ns(self):
        ...
        values = list(root.attrib.values())
        values.sort()
        self.assertEqual(["Bar", "Baz"], values)
    ...
    def test_attribute_contains(self):
        ...
        self.assertEqual(True, "bar" in root.attrib)
        self.assertEqual(False, "baz" in root.attrib)
        self.assertEqual(False, "hah" in root.attrib)
        self.assertEqual(
            True, "{http://ns.codespeak.net/test}baz" in root.attrib
        )
    ...
    def test_del_attribute_ns_parsed(self):
        ...
        a = XML(
            _bytes('<a xmlns:nsa="http://a/" nsa:foo="FooNS" foo="Foo" />')
        )
        ...
        del a.attrib["{http://a/}foo"]
        self.assertRaises(
            KeyError, operator.getitem, a.attrib, "{http://a/}foo"
        )
        self.assertRaises(KeyError, operator.getitem, a.attrib, "foo")

        a = XML(
            _bytes('<a xmlns:nsa="http://a/" foo="Foo" nsa:foo="FooNS" />')
        )
    ...
    def test_XMLID(self):
        ...
        expected = {"chapter1": root[0], "note1": root[1], "warn1": root[4]}
    ...
    def test_itertext(self):
        ...
        XML = self.etree.XML
        root = XML(
            _bytes("<root>RTEXT<a></a>ATAIL<b/><c>CTEXT</c>CTAIL</root>")
        )

        text = list(root.itertext())
        self.assertEqual(["RTEXT", "ATAIL", "CTEXT", "CTAIL"], text)
    ...
    def test_findall_wildcard(self):
        ...
        self.assertEqual(
            summarize_list(root.findall("{*}b")), ["{X}b", "b", "{Y}b"]
        )
        ...
        self.assertEqual(
            summarize_list(root.findall(".//{X}*")), ["{X}b", "{X}b"]
        )
        self.assertEqual(summarize_list(root.findall(".//{Y}*")), ["{Y}b"])
        self.assertEqual(
            summarize_list(root.findall(".//{}*")), ["c", "b", "c", "b"]
        )
    ...
    def test_write_method_html(self):
        ...
        self.assertEqual(
            _bytes("<html><body><p>html<br>test</p></body></html>"), data
        )
    ...
    def test_write_method_text(self):
        ...
        self.assertEqual(_bytes("ABTAILCtail"), data)
    ...
    def test_subelement_reference(self):
        ...
        self.assertEqual("baz2-modified", el[1][0].text)
    ...
    def test_set_text(self):
        ...
        b = SubElement(a, "b")
        a.text = "hoi"
        self.assertEqual("hoi", a.text)
        self.assertEqual("b", a[0].tag)
    ...
    def test_tail1(self):
        ...
        a = Element("a")
        a.tail = "dag"
        self.assertEqual("dag", a.tail)
        b = SubElement(a, "b")
        b.tail = "hoi"
        self.assertEqual("hoi", b.tail)
        self.assertEqual("dag", a.tail)
    ...
    def test_tail_append(self):
        ...
        b.tail = "b_tail"
        a.append(b)
        self.assertEqual("b_tail", b.tail)
    ...
    def test_comment_text(self):
        ...
        self.assertEqual(_bytes("<a><!--foo--></a>"), tostring(a))

        a[0].text = "TEST"
        self.assertEqual(a[0].text, "TEST")

        self.assertEqual(_bytes("<a><!--TEST--></a>"), tostring(a))
    ...
    def test_pi(self):
        ...
        self.assertXML(_bytes("<a><?foo some more text?></a>"), a)
    ...
    def test_setitem(self):
        ...
        self.assertXML(_bytes("<a><c></c></a>"), a)
        self.assertXML(_bytes("<b></b>"), b)
    ...
    def test_setitem_tail(self):
        ...
        a[0] = c
        self.assertXML(_bytes("<a><c></c>C2</a>"), a)
    ...
    def test_tag_write(self):
        ...
        self.assertXML(_bytes("<c><b></b></c>"), a)
    ...
    def test_tag_str_subclass(self):
        ...
        a = Element("a")
        a.tag = strTest("TAG")
        self.assertXML(_bytes("<TAG></TAG>"), a)
    ...
    def test_delitem(self):
        ...
        del a[1]
        self.assertXML(_bytes("<a><b></b><d></d></a>"), a)
        ...
        other = Element("other")
        other.append(c)
        self.assertXML(_bytes("<other><c></c></other>"), other)
    ...
    def test_del_insert(self):
        ...
        el = a[0]
        self.assertXML(_bytes("<a><b><bs></bs></b><c><cs></cs></c></a>"), a)
        self.assertXML(_bytes("<b><bs></bs></b>"), b)
        self.assertXML(_bytes("<c><cs></cs></c>"), c)

        del a[0]
        self.assertXML(_bytes("<a><c><cs></cs></c></a>"), a)
        self.assertXML(_bytes("<b><bs></bs></b>"), b)
        self.assertXML(_bytes("<c><cs></cs></c>"), c)
    ...
    def test_del_setitem(self):
        ...
        del a[0]
        a[0] = el
        self.assertXML(_bytes("<a><b><bs></bs></b></a>"), a)
        self.assertXML(_bytes("<b><bs></bs></b>"), b)
        self.assertXML(_bytes("<c><cs></cs></c>"), c)
    ...
    def test_clear_sub(self):
        ...
        self.assertXML(_bytes("<a></a>"), a)
        self.assertXML(_bytes("<b><c></c></b>"), b)
    ...
    def test_insert(self):
        ...
        self.assertXML(_bytes("<a><d></d><b></b><c></c></a>"), a)

        e = Element("e")
        a.insert(2, e)
        self.assertEqual(e, a[2])
        self.assertXML(_bytes("<a><d></d><b></b><e></e><c></c></a>"), a)
    ...
    def test_insert_name_interning(self):
        ...
        self.assertXML(
            _bytes(
                "<%(a)s><%(d)s></%(d)s><%(b)s></%(b)s><%(c)s></%(c)s></%(a)s>"
                % names
            ),
            a,
        )
        ...
        self.assertXML(
            _bytes(
                "<%(a)s><%(d)s></%(d)s><%(b)s></%(b)s><%(e)s></%(e)s><%(c)s></%(c)s></%(a)s>"
                % names
            ),
            a,
        )
    ...
    def test_insert_beyond_index(self):
        ...
        self.assertXML(_bytes("<a><b></b><c></c></a>"), a)
    ...
    def test_insert_negative(self):
        ...
        self.assertXML(_bytes("<a><b></b><d></d><c></c></a>"), a)
    ...
    def test_insert_tail(self):
        ...
        a.insert(0, c)
        self.assertXML(_bytes("<a><c></c>C2<b></b></a>"), a)
    ...
    def test_remove_ns(self):
        ...
        a.remove(b)
        self.assertXML(
            _bytes('<ns0:a xmlns:ns0="http://test"><ns0:c></ns0:c></ns0:a>'), a
        )
        self.assertXML(_bytes('<ns0:b xmlns:ns0="http://test"></ns0:b>'), b)
    ...
    def test_remove_nonexisting(self):
        ...
        c = SubElement(a, "c")
        d = Element("d")
        self.assertRaises(ValueError, a.remove, d)
    ...
    def test_getchildren(self):
        ...
        d = SubElement(b, "d")
        e = SubElement(c, "e")
        self.assertXML(_bytes("<a><b><d></d></b><c><e></e></c></a>"), a)
        self.assertEqual([b, c], a.getchildren())
        self.assertEqual([d], b.getchildren())
        self.assertEqual([], d.getchildren())
    ...
    def test_makeelement(self):
        ...
        self.assertXML(_bytes('<c hoi="dag"></c>'), b)
    ...
    def test_iter(self):
        ...
        self.assertEqual([a, b, d, c, e], list(a.iter()))
        self.assertEqual([d], list(d.iter()))
    ...
    def test_iter_remove_tail(self):
        ...
        self.assertEqual([None] * 5, [el.tail for el in a.iter()])
    ...
    def test_getiterator(self):
        ...
        self.assertEqual([a, b, d, c, e], list(a.getiterator()))
        self.assertEqual([d], list(d.getiterator()))
    ...
    def test_getiterator_empty(self):
        ...
        self.assertEqual([], list(a.getiterator("none")))
        self.assertEqual([], list(e.getiterator("none")))
        self.assertEqual([e], list(e.getiterator()))
    ...
    def test_getiterator_filter(self):
        ...
        self.assertEqual([a], list(a.getiterator("a")))
        a2 = SubElement(e, "a")
        self.assertEqual([a, a2], list(a.getiterator("a")))
        self.assertEqual([a2], list(c.getiterator("a")))
    ...
    def test_getiterator_filter_comment(self):
        ...
        self.assertEqual([comment_b], list(a.getiterator(Comment)))

        comment_a = Comment("TEST-a")
        a.append(comment_a)

        self.assertEqual([comment_b, comment_a], list(a.getiterator(Comment)))

        self.assertEqual([comment_b], list(b.getiterator(Comment)))
    ...
    def test_getiterator_filter_pi(self):
        ...
        self.assertEqual([pi_b], list(a.getiterator(PI)))

        pi_a = PI("TEST-a")
        a.append(pi_a)

        self.assertEqual([pi_b, pi_a], list(a.getiterator(PI)))

        self.assertEqual([pi_b], list(b.getiterator(PI)))
    ...
    def test_getiterator_filter_with_text(self):
        ...
        a2 = SubElement(e, "a")
        self.assertEqual([a, a2], list(a.getiterator("a")))
        self.assertEqual([a2], list(e.getiterator("a")))
    ...
    def test_getslice(self):
        ...
        self.assertEqual([b, c], a[0:2])
        self.assertEqual([b, c, d], a[:])
        self.assertEqual([b, c, d], a[:10])
        self.assertEqual([b], a[0:1])
        self.assertEqual([], a[10:12])
    ...
    def test_getslice_negative(self):
        ...
        self.assertEqual([d], a[-1:])
        self.assertEqual([c, d], a[-2:])
        self.assertEqual([c], a[-2:-1])
        self.assertEqual([b, c], a[-3:-1])
        self.assertEqual([b, c], a[-3:2])
    ...
    def test_getslice_step(self):
        ...
        self.assertEqual([e, d, c, b], a[::-1])
        self.assertEqual([b, d], a[::2])
        self.assertEqual([e, c], a[::-2])
        self.assertEqual([d, c], a[-2:0:-1])
        self.assertEqual([e], a[:1:-2])
    ...
    def test_comment_getitem_getslice(self):
        ...
        a.append(foo)
        c = SubElement(a, "c")
        self.assertEqual([b, foo, c], a[:])
        self.assertEqual(foo, a[1])
        a[1] = new = Element("new")
        self.assertEqual(new, a[1])
        self.assertXML(_bytes("<a><b></b><new></new><c></c></a>"), a)
    ...
    def test_delslice_child_tail_dealloc(self):
        ...
        a = doc.getroot()
        del a[1:3]
        self.assertXML(_bytes("<a><b></b>B2<e></e>E2</a>"), a)
    ...
    def test_setslice(self):
        ...
        s = [e, f, g]
        a[1:2] = s
        self.assertEqual([b, e, f, g, d], list(a))
    ...
    def test_setslice_all_replace_reversed_ns1(self):
        ...
        self.assertEqual([d, c, b], list(a))
        self.assertEqual(
            ["{ns}d", "{ns}c", "{ns}b"], [child.tag for child in a]
        )
    ...
    def test_setslice_all_replace_reversed_ns2(self):
        ...
        self.assertEqual([d, c, b], list(a))
        self.assertEqual(
            ["{ns3}d", "{ns2}c", "{ns1}b"], [child.tag for child in a]
        )
    ...
    def test_setslice_end(self):
        ...
        s = [e, f]
        a[99:] = s
        self.assertEqual([b, c, e, f], list(a))

        s = [g, h]
        a[:0] = s
        self.assertEqual([g, h, b, c, e, f], list(a))
    ...
    def test_setslice_end_exact(self):
        ...
        s = [e, f, g]
        a[3:] = s
        self.assertEqual([b, c, d, e, f, g], list(a))
    ...
    def test_setslice_single(self):
        ...
        s = [e]
        a[0:1] = s
        self.assertEqual([e, c], list(a))

        s = [f]
        a[1:2] = s
        self.assertEqual([e, f], list(a))
    ...
    def test_setslice_tail(self):
        ...
        a[1:3] = [x, y, z]
        self.assertXML(
            _bytes("<a><b></b>B2<x></x>X2<y></y>Y2<z></z>Z2<e></e>E2</a>"), a
        )
    ...
    def test_setslice_negative(self):
        ...
        a[1:-1] = [x, y]
        self.assertEqual([b, x, y, d], list(a))
    ...
    def test_setslice_negative2(self):
        ...
        a[1:-2] = [x, y]
        self.assertEqual([b, x, y, c, d], list(a))
    ...
    def test_elementtree_getiterator(self):
        ...
        self.assertEqual([a, b, d, c, e], list(t.getiterator()))
    ...
    def test_elementtree_getiterator_filter(self):
        ...
        self.assertEqual([a], list(t.getiterator("a")))
        a2 = SubElement(e, "a")
        self.assertEqual([a, a2], list(t.getiterator("a")))
    ...
    def test_ns_access(self):
        ...
        a = t.getroot()
        self.assertEqual("{%s}a" % ns, a.tag)
        self.assertEqual("{%s}b" % ns, a[0].tag)
    ...
    def test_ns_access2(self):
        ...
        f = BytesIO(
            '<x:a xmlns:x="%s" xmlns:y="%s"><x:b></x:b><y:b></y:b></x:a>'
            % (ns, ns2)
        )
        t = ElementTree(file=f)
        a = t.getroot()
        self.assertEqual("{%s}a" % ns, a.tag)
        self.assertEqual("{%s}b" % ns, a[0].tag)
        self.assertEqual("{%s}b" % ns2, a[1].tag)
    ...
    def test_ns_setting(self):
        ...
        self.assertEqual("{%s}a" % ns, a.tag)
        self.assertEqual("{%s}b" % ns2, b.tag)
        self.assertEqual("{%s}c" % ns, c.tag)
        self.assertEqual("{%s}a" % ns, a.tag)
        self.assertEqual("{%s}b" % ns2, b.tag)
        self.assertEqual("{%s}c" % ns, c.tag)
    ...
    def test_ns_tag_parse(self):
        ...
        ns2 = "http://xml.infrae.com/2"
        f = BytesIO(
            '<a xmlns="%s" xmlns:x="%s"><x:b></x:b><b></b></a>' % (ns, ns2)
        )
        t = ElementTree(file=f)

        a = t.getroot()
        self.assertEqual("{%s}a" % ns, a.tag)
        self.assertEqual("{%s}b" % ns2, a[0].tag)
        self.assertEqual("{%s}b" % ns, a[1].tag)
    ...
    def test_ns_attr(self):
        ...
        a.set("{%s}foo" % ns, "Foo")
        a.set("{%s}bar" % ns2, "Bar")
        self.assertEqual("Foo", a.get("{%s}foo" % ns))
        self.assertEqual("Bar", a.get("{%s}bar" % ns2))
        try:
            self.assertXML(
                _bytes(
                    '<a xmlns:ns0="%s" xmlns:ns1="%s" ns0:foo="Foo" ns1:bar="Bar"></a>'
                    % (ns, ns2)
                ),
                a,
            )
            ...
            self.assertXML(
                _bytes(
                    '<a xmlns:ns0="%s" xmlns:ns1="%s" ns1:foo="Foo" ns0:bar="Bar"></a>'
                    % (ns2, ns)
                ),
                a,
            )
    ...
    def test_ns_decl_tostring(self):
        ...
        nsdecl = re.findall(
            _bytes("xmlns(?::[a-z0-9]+)?=[\"']([^\"']+)[\"']"), tostring(baz)
        )
    ...
    def test_attribute_xmlns_move(self):
        ...
        subelement = Element(
            "subelement", {"{http://www.w3.org/XML/1998/namespace}id": "foo"}
        )
        self.assertEqual(1, len(subelement.attrib))
        self.assertEqual(
            "foo", subelement.get("{http://www.w3.org/XML/1998/namespace}id")
        )
    ...
    def test_attribute_namespace_roundtrip(self):
        ...
        ns_href = "http://a.b.c"
        xml = _bytes(
            '<root xmlns="%s" xmlns:x="%s"><el x:a="test" /></root>'
            % (ns_href, ns_href)
        )
        root = fromstring(xml)
    ...
    def test_register_namespace(self):
        ...
        self.assertEqual(
            _bytes('<ns0:test xmlns:ns0="%s"></ns0:test>' % namespace),
        ...
        self.assertRaises(
            ValueError, self.etree.register_namespace, "ns25", namespace
        )
    ...
    def test_tostring(self):
        ...
        self.assertEqual(
            _bytes("<a><b></b><c></c></a>"), canonicalize(tostring(a))
        )
    ...
    def test_tostring_element(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(c, "d")
        self.assertEqual(_bytes("<b></b>"), canonicalize(tostring(b)))
        self.assertEqual(_bytes("<c><d></d></c>"), canonicalize(tostring(c)))
    ...
    def test_tostring_method_html(self):
        ...
        self.assertEqual(
            _bytes("<html><body><p>html<br>test</p></body></html>"),
    ...
    def test_tostring_method_text(self):
        ...
        self.assertEqual(_bytes("ABTAILCtail"), tostring(a, method="text"))
    ...
    def test_iterparse_file(self):
        ...
        events = list(iterator)
        root = iterator.root
        self.assertEqual([("end", root[0]), ("end", root)], events)
    ...
    def test_iterparse_set_ns_attribute(self):
        iterparse = self.etree.iterparse
        f = BytesIO(
            '<a xmlns="http://ns1/"><b><c xmlns="http://ns2/"/></b></a>'
        )
    ...
    def test_iterparse_move_elements(self):
        ...
        for event, node in etree.iterparse(f):
            pass

        root = etree.Element("new_root", {})
        root[:] = node[:]

        self.assertEqual(["b", "c"], [el.tag for el in root])
    ...
    def test_iterparse_cdata(self):
        ...
        self.assertEqual(_bytes("<root>test</root>"), tostring(context.root))

    def test_parse_file(self):
        parse = self.etree.parse
        # from file
        tree = parse(fileInTestDir("test.xml"))
        self.assertXML(_bytes("<a><b></b></a>"), tree.getroot())
    ...
    def test_parse_with_encoding(self):
        ...
        self.assertXML(_bytes("<html></html>"), tree.getroot())
    ...
    def test_encoding(self):
        ...
        self.assertXML(
            _str("<a>Søk på nettet</a>").encode("UTF-8"), a, "utf-8"
        )
    ...
    def test_encoding_8bit_latin1(self):
        ...
        result = result.split(_bytes("?>"), 1)[-1].replace(
            _bytes("\n"), _bytes("")
        )
        self.assertEqual(
            _str("<a>Søk på nettet</a>").encode("iso-8859-1"), result
        )
    ...
    def test_parse_encoding_8bit_explicit(self):
        ...
        self.assertRaises(
            self.etree.ParseError, self.etree.parse, BytesIO(xml_latin1)
        )

        tree = self.etree.parse(
            BytesIO(xml_latin1), XMLParser(encoding="iso-8859-1")
        )
        a = tree.getroot()
    ...
    def _test_wrong_unicode_encoding(self):
        ...
        XML = self.etree.XML
        test_utf = _str('<?xml version="1.0" encoding="iso-8859-1"?>') + _str(
            "<a>Søk på nettet</a>"
        )
    ...
    def test_encoding_tostring_unknown(self):
        ...
        a.text = _str("Søk på nettet")
        self.assertRaises(
            LookupError, tostring, a, encoding="Invalid Encoding"
        )
    ...
    def test_encoding_tostring_sub(self):
        ...
        self.assertEqual(
            _str("<b>Søk på nettet</b>").encode("UTF-8"),
    ...
    def test_encoding_tostring_sub_tail(self):
        ...
        b.tail = _str("Søk")
        self.assertEqual(
            _str("<b>Søk på nettet</b>Søk").encode("UTF-8"),
            tostring(b, encoding="utf-8"),
        )
    ...
    def test_encoding_tostring_default_encoding(self):
        ...
        self.assertEqual(expected, tostring(a))
    ...
    def test_encoding_sub_tostring_default_encoding(self):
        ...
        self.assertEqual(expected, tostring(b))
    ...
    def test_deepcopy_append(self):
        ...
        self.assertEqual(
            _bytes("<a><C/></a>"), tostring(a).replace(_bytes(" "), _bytes(""))
        )
        self.assertEqual(
            _bytes("<a><X/></a>"), tostring(b).replace(_bytes(" "), _bytes(""))
        )
    ...
    def test_qname_attribute_resolve(self):
        ...
        self.assertXML(
            _bytes('<ns0:a xmlns:ns0="http://myns" ns0:a="ns0:a"></ns0:a>'), a
        )
    ...
    def test_qname_attribute_resolve_new(self):
        ...
        self.assertXML(_bytes('<a xmlns:ns0="http://myns" a="ns0:a"></a>'), a)
    ...
    def test_parser_target_attrib(self):
        ...
        self.assertEqual("DONE", done)
        self.assertEqual(
            ["start-root", "start-sub", "end-sub", "end-root"], events
        )
    ...
    def test_parser_target_end_ns(self):
        ...
        self.assertEqual(builder, [("end-ns", "p"), ("end-ns", ""),])
    ...
    def test_late_tail_mix_pi_comments(self):
        ...
        parser = ET.XMLParser(
            target=ET.TreeBuilder(insert_comments=True, insert_pis=False)
        )
        parser.feed(xml)
        a = parser.close()
        ...
        parser = ET.XMLParser(
            target=TreeBuilderSubclass(insert_comments=True, insert_pis=False)
        )
        parser.feed(xml)
        a = parser.close()
        self.assertEqual(a[0].text, " comment ")
        ...
        xml = "<a>text<!-- comment -->\n<?pi data?>tail</a>"
        parser = ET.XMLParser(
            target=ET.TreeBuilder(insert_pis=True, insert_comments=False)
        )
        parser.feed(xml)
        a = parser.close()
        ...
        parser = ET.XMLParser(
            target=TreeBuilderSubclass(insert_pis=True, insert_comments=False)
        )
        parser.feed(xml)
        a = parser.close()
        self.assertEqual(a[0].text[-4:], "data")
    ...
    def _rootstring(self, tree):
        return (
            self.etree.tostring(tree.getroot())
...
class _ElementSlicingTest(unittest.TestCase):
    ...
    def test_setslice_steps(self):
        ...
        self.assertEqual(
            self._subelem_tags(e), ["a0", "b0", "a2", "b1", "a4", "a5"]
        )
        ...
        with self.assertRaises(ValueError):
            e[1:5:2] = []
        self.assertEqual(
            self._subelem_tags(e), ["a0", "a1", "a2", "a3", "a4", "a5"]
        )
...
class _XMLPullParserTest(unittest.TestCase):
    ...
    def assert_event_tuples(self, parser, expected, max_events=None):
        self.assertEqual(
            list(islice(parser.read_events(), max_events)), expected
        )
    ...
    def test_simple_xml(self):
        for chunk_size in (None, 1, 5):
            ...
            self.assert_event_tags(
                parser, [("end", "element"), ("end", "empty-element"),]
            )
    ...
    def test_ns_events(self):
        ...
        self._feed(parser, "<root xmlns='namespace'>\n")
        self.assertEqual(
            list(parser.read_events()), [("start-ns", ("", "namespace"))]
        )
        self._feed(parser, "<element key='value'>text</element")
        self._feed(parser, ">\n")
        self._feed(parser, "<element>text</element>tail\n")
    ...
    def test_ns_events_end_ns_only(self):
        ...
        self._feed(parser, "<!-- comment -->\n")
        self._feed(
            parser, "<root xmlns='namespace' xmlns:a='abc' xmlns:b='xyz'>\n"
        )
        ...
        self.assertEqual(
            ...
            [("end-ns", None), ("end-ns", None), ("end-ns", None),],
        )
        parser.close()
    ...
    def test_ns_events_start(self):
        ...
        self.assert_event_tags(
            parser, [("start", "{abc}child"), ("end", "{abc}child"),]
        )
    ...
    def test_ns_events_start_end(self):
        parser = self.etree.XMLPullParser(
            events=("start-ns", "start", "end", "end-ns")
        )
        self._feed(parser, "<tag xmlns='abc' xmlns:p='xyz'>\n")
        self.assert_event_tuples(
            parser,
    ...
    def test_events(self):
        ...
        self.assert_event_tags(parser, [("end", "element")])
        self._feed(
            parser, "<element xmlns='foo'>text<empty-element/></element>tail\n"
        )
        self.assert_event_tags(
            parser,
            [
        ...
        self.assert_event_tags(
            parser,
            [("start", "{foo}element"), ("start", "{foo}empty-element"),],
        )
        self._feed(parser, "</root>")
        root = self._close_and_return_root(parser)
        self.assertEqual(root.tag, "root")
...
class _C14NTest(unittest.TestCase):
    ...
    def test_simple_roundtrip(self):
        ...
        self.assertEqual(
            c14n_roundtrip("<doc xmlns='uri'/>"),  # FIXME
            '<doc xmlns="uri"></doc>',
        )
        self.assertEqual(
        ...
        self.assertEqual(
            c14n_roundtrip("<value>&#x32;</value>"), "<value>2</value>"
        )
        self.assertEqual(
            c14n_roundtrip(
    ...
    def test_c14n_exclusion(self):
        ...
        self.assertEqual(
            c14n_roundtrip(
                xml,
                strip_text=True,
                exclude_tags=["{http://example.com/x}d", "b"],
            ),
            "<root>"
    ...
    def test_xml_c14n2(self):
        ...
        input_files = [
            filename for filename in files if filename.startswith("in")
        ]
        configs = {
            filename: {
                ...
                for option in self.etree.parse(
                    full_path(filename) + ".xml"
                ).getroot()
            }
        ...
        for input_file, output_files in tests.items():
            for output_file, config in output_files:
                ...
                rewrite_prefixes = (
                    get_option(config, "PrefixRewrite") == "sequential"
                )
                ...
                config_descr = ",".join(
                    ...
                    % (
                        name,
                        value
                        or ",".join(c.tag.split("}")[-1] for c in children),
                    )
                ...
                with self.subTest("{}({})".format(output_file, config_descr)):
                    ...
                    if (
                        input_file == "inNsSuperfluous"
                        and not rewrite_prefixes
                    ):
                        self.skipTest(
                            "Redeclared namespace handling is not supported in {}".format(
                                output_file
        ...
        def _canonicalize(
            ...
            **options
            ...
            if rewrite_prefixes or qname_aware_attrs or qname_aware_tags:
                self.skipTest(
                    "C14N 2.0 feature not supported with ElementTree.write()"
                )

            parser = self.etree.XMLParser(
                attribute_defaults=True, collect_ids=False
            )
    ...
    class ETreeC14N2TostringTest(ETreeC14NTest):
        def _canonicalize(
            ...
            **options
            ...
            if rewrite_prefixes or qname_aware_attrs or qname_aware_tags:
                self.skipTest(
                    "C14N 2.0 feature not supported with ElementTree.tostring()"
                )
...
if ElementTree:
    ...
    filter_by_version(
        ElementTreeTestCase,
        ElementTreeTestCase.required_versions_ET,
        ET_VERSION,
    )
...
if cElementTree:
    ...
    filter_by_version(
        ...
        CElementTreeTestCase.required_versions_cET,
        CET_VERSION,
    )
