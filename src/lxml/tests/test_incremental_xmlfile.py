...
class _XmlFileTestCaseBase(HelperTestCase):
    ...
    def test_element_nested_with_text(self):
        with etree.xmlfile(self._file) as xf:
            with xf.element("test"):
                ...
                xf.write("noc")
        self.assertXml(
            "<test>con<toast>tent<taste>inside</taste>"
            "tnet</toast>noc</test>"
        )
        ...
        with etree.xmlfile(self._file) as xf:
            with xf.element("test"):
                with xf.element("{nsURI}toast"):
                    pass
        self.assertXml(
            '<test><ns0:toast xmlns:ns0="nsURI"></ns0:toast></test>'
        )
    ...
    def test_buffering(self):
        with etree.xmlfile(self._file, buffered=False) as xf:
            ...
            self.assertXml(
                "<test>toast<taste>some<more/>toast</taste>end</test>"
            )
    ...
    def test_generator_close_continues_closing(self):
        ...
        g.close()
        self.assertXml(
            "<root><entry>A</entry><entry>B</entry><entry>C</entry></root>"
        )
...
@skipIf(
    sys.platform.startswith("win"), "Can't reopen temporary files on Windows"
)
...
class SimpleFileLikeXmlFileTestCase(_XmlFileTestCaseBase):
    ...
    def test_write_fails(self):
        ...
        for trigger in ["text", "root", "tag", "noflush"]:
            try:
                with etree.xmlfile(
                    Writer(trigger.encode("utf8")), encoding="utf8"
                ) as xf:
                    with xf.element("root"):
                        xf.flush()
                        with xf.element("tag"):
                ...
                self.assertTrue(
                    False, "exception not raised for '%s'" % trigger
                )
...
class HtmlFileTestCase(_XmlFileTestCaseBase):
    ...
    def test_method_context_manager_misuse(self):
        with etree.htmlfile(self._file) as xf:
            with xf.element("foo"):
                ...
                self.assertRaises(
                    LxmlSyntaxError, cm2.__exit__, None, None, None
                )
                ...
                with xf.method("html"):
                    self.assertRaises(
                        LxmlSyntaxError, cm3.__exit__, None, None, None
                    )
    ...
    def test_attribute_quoting_unicode(self):
        with etree.htmlfile(self._file) as xf:
            with xf.element(
                "tagname",
                attrib={"attr": _str('"misquöted\\u3344\\U00013344"')},
            ):
                xf.write("foo")
...
class AsyncXmlFileTestCase(HelperTestCase):
    def test_async_api(self):
        ...
        def api_of(obj):
            return sorted(
                name for name in dir(scm) if not name.startswith("__")
            )
    ...
    @skipIf(
        sys.version_info < (3, 5), "requires support for async-def (Py3.5+)"
    )
    def test_async(self):
        ...
        data, unbuffered_calls = self._run_async(
            test_async_xmlfile(buffered=False)
        )
        self.assertEqual(expected, data)
