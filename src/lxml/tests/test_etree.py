...
class ETreeOnlyTestCase(HelperTestCase):
    ...
    def test_version(self):
        ...
        self.assertTrue(
            etree.__version__.startswith(str(etree.LXML_VERSION[0]))
        )
    ...
    def test_include_paths(self):
        ...
        self.assertTrue(
            os.path.join(os.path.dirname(lxml.__file__), "includes")
            in includes,
            includes,
        )
    ...
    def test_nsmap_prefix_invalid(self):
        etree = self.etree
        self.assertRaises(
            ValueError, etree.Element, "root", nsmap={'"': "testns"}
        )
        self.assertRaises(
            ValueError, etree.Element, "root", nsmap={"&": "testns"}
        )
        self.assertRaises(
            ValueError, etree.Element, "root", nsmap={"a:b": "testns"}
        )
    ...
    def test_clear_keep_tail(self):
        ...
        self.assertEqual(
            _bytes('<a aa="A"><b/>B2<c ca="C">C1</c>C2</a>'), tostring(a)
        )
    ...
    def test_attribute_has_key(self):
        ...
        root = XML(
            _bytes(
                '<foo bar="Bar" xmlns:ns="http://ns.codespeak.net/test" ns:baz="Baz" />'
            )
        )
        self.assertEqual(True, root.attrib.has_key("bar"))
        self.assertEqual(False, root.attrib.has_key("baz"))
        self.assertEqual(False, root.attrib.has_key("hah"))
        self.assertEqual(
            True, root.attrib.has_key("{http://ns.codespeak.net/test}baz")
        )
        ...
        for dict_type in ordered_dict_types:
            ...
            try:
                ...
                self.assertSequenceEqual(
                    expected_values, root2.attrib.values()
                )
                self.assertSequenceEqual(expected_items, root2.attrib.items())
            except AssertionError as exc:
                exc.args = (
                    "Order of '%s': %s" % (dict_type.__name__, exc.args[0]),
                ) + exc.args[1:]
                raise
    ...
    def test_strip_attributes(self):
        ...
        self.assertEqual(
            _bytes('<test b="10" c="20"><x b="2"></x></test>'),
    ...
    def test_strip_attributes_ns(self):
        XML = self.etree.XML
        xml = _bytes(
            '<test xmlns:n="http://test/ns" a="6" b="10" c="20" n:a="5"><x a="4" n:b="2"/></test>'
        )
        ...
        self.assertEqual(
            _bytes(
                '<test xmlns:n="http://test/ns" b="10" c="20" n:a="5"><x n:b="2"></x></test>'
            ),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes(
                '<test xmlns:n="http://test/ns" a="6" b="10"><x a="4" n:b="2"></x></test>'
            ),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes(
                '<test xmlns:n="http://test/ns" a="6" b="10" c="20"><x a="4"></x></test>'
            ),
            self._writeElement(root),
        )
    ...
    def test_strip_elements(self):
        ...
        self.assertEqual(
            _bytes("<test><a><b></b></a><x><a><b></b></a></x></test>"),
            self._writeElement(root),
        )
    ...
    def test_strip_elements_ns(self):
        XML = self.etree.XML
        xml = _bytes(
            '<test>TEST<n:a xmlns:n="urn:a">A<b>B<c xmlns="urn:c"/>C</b>BT</n:a>AT<x>X<a>A<b xmlns="urn:a"/>BT<c xmlns="urn:x"/>CT</a>AT</x>XT</test>'
        )
        ...
        self.assertEqual(
            _bytes(
                '<test>TEST<n:a xmlns:n="urn:a">A<b>B<c xmlns="urn:c"></c>C</b>BT</n:a>AT<x>X</x>XT</test>'
            ),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes(
                '<test>TEST<n:a xmlns:n="urn:a">A<b>B<c xmlns="urn:c"></c>C</b>BT</n:a>AT<x>X<a>A<c xmlns="urn:x"></c>CT</a>AT</x>XT</test>'
            ),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes(
                '<test>TEST<x>X<a>A<c xmlns="urn:x"></c>CT</a>AT</x>XT</test>'
            ),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes(
                '<test>TESTAT<x>X<a>ABT<c xmlns="urn:x"></c>CT</a>AT</x>XT</test>'
            ),
            self._writeElement(root),
        )
    ...
    def test_strip_tags(self):
        XML = self.etree.XML
        xml = _bytes(
            "<test>TEST<a>A<b>B<c/>CT</b>BT</a>AT<x>X<a>A<b/>BT<c/>CT</a>AT</x>XT</test>"
        )
        ...
        self.assertEqual(
            _bytes(
                "<test>TESTA<b>B<c></c>CT</b>BTAT<x>XA<b></b>BT<c></c>CTAT</x>XT</test>"
            ),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes("<test>TEST<a>ABCTBT</a>AT<x>X<a>ABTCT</a>AT</x>XT</test>"),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes(
                "<test>TEST<a>A<b>BCT</b>BT</a>AT<x>X<a>A<b></b>BTCT</a>AT</x>XT</test>"
            ),
            self._writeElement(root),
        )
    ...
    def test_strip_tags_doc_style(self):
        ...
        self.etree.strip_tags(root, "a")
        self.assertEqual(
            re.sub(_bytes("</?a[^>]*>"), _bytes(""), xml).replace(
                _bytes("<br/>"), _bytes("<br></br>")
            ),
            self._writeElement(root),
        )
    ...
    def test_strip_tags_ns(self):
        XML = self.etree.XML
        xml = _bytes(
            '<test>TEST<n:a xmlns:n="urn:a">A<b>B<c xmlns="urn:c"/>CT</b>BT</n:a>AT<x>X<a>A<b xmlns="urn:a"/>BT<c xmlns="urn:x"/>CT</a>AT</x>XT</test>'
        )
        ...
        self.assertEqual(
            _bytes(
                '<test>TEST<n:a xmlns:n="urn:a">A<b>B<c xmlns="urn:c"></c>CT</b>BT</n:a>AT<x>XA<b xmlns="urn:a"></b>BT<c xmlns="urn:x"></c>CTAT</x>XT</test>'
            ),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes(
                '<test>TEST<n:a xmlns:n="urn:a">A<b>B<c xmlns="urn:c"></c>CT</b>BT</n:a>AT<x>X<a>ABT<c xmlns="urn:x"></c>CT</a>AT</x>XT</test>'
            ),
            self._writeElement(root),
        )
        ...
        self.assertEqual(
            _bytes(
                '<test>TESTA<b>B<c xmlns="urn:c"></c>CT</b>BTAT<x>X<a>ABT<c xmlns="urn:x"></c>CT</a>AT</x>XT</test>'
            ),
            self._writeElement(root),
        )
    ...
    def test_strip_tags_and_remove(self):
        ...
        root = HTML(_bytes("<div><h1>title</h1> <b>foo</b> <p>boo</p></div>"))[
            0
        ][0]
        self.assertEqual(
            _bytes("<div><h1>title</h1> <b>foo</b> <p>boo</p></div>"),
            self.etree.tostring(root),
        )
        self.etree.strip_tags(root, "b")
        self.assertEqual(
            _bytes("<div><h1>title</h1> foo <p>boo</p></div>"),
            self.etree.tostring(root),
        )
        root.remove(root[0])
        self.assertEqual(
            _bytes("<div><p>boo</p></div>"), self.etree.tostring(root)
        )
    ...
    def test_pi_pseudo_attributes_get(self):
        ...
        root = XML(
            _bytes(
                "<test><?mypi my='1' test=\" abc \" quotes=\"' '\" only names ?></test>"
            )
        )
        self.assertEqual(root[0].target, "mypi")
    ...
    def test_deepcopy_elementtree_pi(self):
        ...
        self.assertEqual(
            _bytes("<?mypi my test ?><test/><!--comment -->"), tostring(tree1)
        )

        tree2 = copy.deepcopy(tree1)
        self.assertEqual(
            _bytes("<?mypi my test ?><test/><!--comment -->"), tostring(tree2)
        )

        root2 = copy.deepcopy(tree1.getroot())
        self.assertEqual(_bytes("<test/>"), tostring(root2))
    ...
    def test_parse_remove_comments(self):
        ...
        self.assertEqual(_bytes("<a><b><c/></b></a>"), tostring(root))
    ...
    def test_parse_remove_pis(self):
        ...
        f = BytesIO(xml)
        tree = parse(f)
        self.assertEqual(xml, tostring(tree))

        parser = XMLParser(remove_pis=True)
        tree = parse(f, parser)
        self.assertEqual(_bytes("<a><b><c/></b></a>"), tostring(tree))
    ...
    def test_iterparse_comments(self):
        ...
        self.assertEqual(
            ["A", " B ", "c", "b", "C", "a"], [name(*item) for item in events]
        )
        self.assertEqual(
            _bytes("<a><!--A--><b><!-- B --><c/></b><!--C--></a>"),
            tostring(root),
    ...
    def test_iterparse_remove_comments(self):
        ...
        f = BytesIO("<a><!--A--><b><!-- B --><c/></b><!--C--></a>")
        events = list(
            iterparse(f, remove_comments=True, events=("end", "comment"))
        )
        root = events[-1][1]
        self.assertEqual(3, len(events))
        self.assertEqual(["c", "b", "a"], [el.tag for (event, el) in events])
        self.assertEqual(_bytes("<a><b><c/></b></a>"), tostring(root))
    ...
    def test_iterparse_strip(self):
        ...
        text = [(element.text, element.tail) for event, element in iterator]
        self.assertEqual(
            [(" b test ", None), (" \n ", None), (None, None)], text
        )
    ...
    def test_iterparse_tag(self):
        ...
        root = iterator.root
        self.assertEqual([("start", root[0]), ("end", root[0])], events)
    ...
    def test_iterparse_encoding_error(self):
        ...
        xml_latin1 = (_str("%s<a>%s</a>") % (wrong_declaration, text)).encode(
            "iso-8859-1"
        )

        self.assertRaises(
            self.etree.ParseError,
            list,
            self.etree.iterparse(BytesIO(xml_latin1)),
        )
    ...
    def test_iterparse_encoding_8bit_override(self):
        ...
        iterator = self.etree.iterparse(
            BytesIO(xml_latin1), encoding="iso-8859-1"
        )
    ...
    def test_iterparse_keep_cdata(self):
        ...
        self.assertEqual(
            _bytes("<root><![CDATA[test]]></root>"), tostring(context.root)
        )
    ...
    def test_elementtree_parser_target_type_error(self):
        ...
        self.assertRaises(
            TypeError, tree.parse, BytesIO("<TAG/>"), parser=parser
        )
    ...
    def test_parser_target_feed_exception(self):
        ...
        self.assertEqual(
            ["start-root", "data-A", "start-a", "data-ca", "end-a", "close"],
            events,
        )
    ...
    def test_parser_target_fromstring_exception(self):
        ...
        try:
            done = self.etree.fromstring(
                _bytes("<root>A<a>ca</a>B</root>"), parser=parser
            )
            self.fail("error expected, but parsing succeeded")
        except ValueError:
            done = "value error received as expected"
    ...
    def test_parser_target_comment(self):
        ...
        parser.feed(
            _bytes("<!--a--><root>A<!--b--><sub/><!--c-->B</root><!--d-->")
        )
        done = parser.close()
    ...
    def test_parser_target_cdata(self):
        ...
        parser = self.etree.XMLParser(target=Target(), strip_cdata=False)
    ...
    def test_parser_target_recover(self):
        ...
        parser = self.etree.XMLParser(target=Target(), recover=True)
    ...
    def test_iterwalk(self):
        ...
        self.assertEqual(
            [("end", root[0]), ("end", root[1]), ("end", root)], events
        )
    ...
    def test_iterwalk_comments_tree(self):
        ...
        iterator = iterwalk(
            self.etree.ElementTree(root), events=("start", "end", "comment")
        )
        events = list(iterator)
        self.assertEqual(
            [
    ...
    def test_iterwalk_pis_tree(self):
        ...
        iterator = iterwalk(
            self.etree.ElementTree(root), events=("start", "end", "pi")
        )
        events = list(iterator)
        self.assertEqual(
            [
    ...
    def test_iterwalk_pis_comments_tree(self):
        ...
        iterator = iterwalk(
            self.etree.ElementTree(root),
            events=("start", "end", "pi", "comment"),
        )
        events = list(iterator)
        self.assertEqual(
            [
    ...
    def test_iterwalk_pis_comments_tree_no_events(self):
        ...
        iterator = iterwalk(
            self.etree.ElementTree(root), events=("start", "end")
        )
        events = list(iterator)
        self.assertEqual(
            [
    ...
    def test_iterwalk_start(self):
        ...
        events = list(iterator)
        self.assertEqual(
            [("start", root), ("start", root[0]), ("start", root[1])], events
        )
    ...
    def test_iterwalk_start_tags(self):
        ...
        events = list(iterator)
        self.assertEqual([("start", root[0]), ("start", root[2])], events)
    ...
    def test_iterwalk_attrib_ns(self):
        iterwalk = self.etree.iterwalk
        root = self.etree.XML(
            _bytes('<a xmlns="ns1"><b><c xmlns="ns2"/></b></a>')
        )

        attr_name = "{testns}bla"
        events = []
        iterator = iterwalk(
            root, events=("start", "end", "start-ns", "end-ns")
        )
        for event, elem in iterator:
            events.append(event)
            if event == "start":
        ...
        self.assertEqual(None, root.get(attr_name))
        self.assertEqual("value", root[0].get(attr_name))
    ...
    def test_iterwalk_getiterator(self):
        ...
        self.assertEqual([1, 2, 1, 4], counts)
    ...
    def test_itertext_comment_pi(self):
        ...
        self.assertEqual(["RTEXT", "ATAIL", "CTAIL", " PITAIL "], text)
    ...
    def test_resolve_string_dtd(self):
        ...
        xml = (
            _str('<!DOCTYPE doc SYSTEM "%s"><doc>&myentity;</doc>') % test_url
        )
    ...
    def test_resolve_filename_dtd(self):
        ...
        tree = parse(StringIO(xml), parser)
        root = tree.getroot()
        self.assertEqual(root.attrib, {"default": "valueA"})
        self.assertEqual(root[0].attrib, {"default": "valueB"})
    ...
    def test_resolve_filename_dtd_relative(self):
        ...
        xml = _str('<!DOCTYPE a SYSTEM "%s"><a><b/></a>') % test_url
        tree = parse(
            StringIO(xml), parser, base_url=fileUrlInTestDir("__test.xml")
        )
        root = tree.getroot()
        self.assertEqual(root.attrib, {"default": "valueA"})
        self.assertEqual(root[0].attrib, {"default": "valueB"})
    ...
    def test_entity_parse(self):
        ...
        self.assertEqual(_bytes("<doc>&myentity;</doc>"), tostring(root))
    ...
    def test_entity_restructure(self):
        ...
        self.assertEqual(
            [el.tag for el in root], ["child1", "child2", "child3"]
        )

        root[0] = root[-1]
        self.assertEqual([el.tag for el in root], ["child3", "child2"])
    ...
    def test_entity_append(self):
        ...
        self.assertEqual(_bytes("<root>&test;</root>"), tostring(root))
    ...
    def test_cdata_tail(self):
        ...
        self.assertEqual(_bytes("<root/><![CDATA[test]]>"), tostring(root))
    ...
    def test_cdata_errors(self):
        ...
        self.assertRaises(TypeError, root.set, "attr", cdata)
        self.assertRaises(
            TypeError, operator.setitem, root.attrib, "attr", cdata
        )
    ...
    def test_setitem_assert(self):
        ...
        self.assertRaises(TypeError, a.__setitem__, 0, "foo")
    ...
    def test_addnext(self):
        ...
        self.assertEqual(["a", "b"], [c.tag for c in root])
        root[1].addnext(root[0])
        self.assertEqual(["b", "a"], [c.tag for c in root])
    ...
    def test_addprevious_pi(self):
        ...
        self.assertEqual(
            _bytes("<root><?TARGET TEXT?>TAIL<a></a></root>"),
            self._writeElement(root),
        )
    ...
    def test_addprevious_root_pi(self):
        ...
        self.assertEqual(_bytes("<root></root>"), self._writeElement(root))
        root.addprevious(pi)
        self.assertEqual(
    ...
    def test_addnext_pi(self):
        ...
        root[0].addnext(pi)
        self.assertEqual(
            _bytes("<root><a></a><?TARGET TEXT?>TAIL</root>"),
            self._writeElement(root),
        )
    ...
    def test_addnext_comment(self):
        ...
        self.assertEqual(
            _bytes("<root><a></a><!--TEXT -->TAIL</root>"),
            self._writeElement(root),
        )
    ...
    def test_addprevious_comment(self):
        ...
        self.assertEqual(
            _bytes("<root><!--TEXT -->TAIL<a></a></root>"),
            self._writeElement(root),
        )
    ...
    def test_comment_empty(self):
        ...
        a = Element("a")
        a.append(Comment())
        self.assertEqual(_bytes("<a><!----></a>"), self._writeElement(a))
    ...
    def test_comment_parse_empty(self):
        ...
        a = doc.getroot()
        self.assertEqual("", a[1].text)
        self.assertEqual(xml, tostring(a))
    ...
    def test_prefix(self):
        ...
        doc = ElementTree(file=f)
        a = doc.getroot()
        self.assertEqual(None, a.prefix)
        self.assertEqual("foo", a[0].prefix)
    ...
    def test_prefix_default_ns(self):
        ...
        a = doc.getroot()
        self.assertEqual(None, a.prefix)
        self.assertEqual(None, a[0].prefix)
    ...
    def test_getparent(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(b, "d")
        self.assertEqual(None, a.getparent())
        self.assertEqual(a, b.getparent())
        self.assertEqual(b.getparent(), c.getparent())
        self.assertEqual(b, d.getparent())
    ...
    def test_iterchildren_tag_multiple(self):
        ...
        root = XML(
            _bytes("<doc><one/><two>Two</two>Hm<two>Bla</two><three/></doc>")
        )
        result = []
    ...
    def test_iterancestors(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(b, "d")
        self.assertEqual([], list(a.iterancestors()))
        self.assertEqual([a], list(b.iterancestors()))
        self.assertEqual([a], list(c.iterancestors()))
        self.assertEqual([b, a], list(d.iterancestors()))
    ...
    def test_iterancestors_tag(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(b, "d")
        self.assertEqual([a], list(d.iterancestors("a")))
        self.assertEqual([a], list(d.iterancestors(tag="a")))

        self.assertEqual([b, a], list(d.iterancestors("*")))
        self.assertEqual([b, a], list(d.iterancestors(tag="*")))
    ...
    def test_iterancestors_tag_multiple(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(b, "d")
        self.assertEqual([b, a], list(d.iterancestors(tag=("a", "b"))))
        self.assertEqual([b, a], list(d.iterancestors("a", "b")))

        self.assertEqual([], list(d.iterancestors(tag=("w", "x", "y", "z"))))
        self.assertEqual([], list(d.iterancestors("w", "x", "y", "z")))

        self.assertEqual([], list(d.iterancestors(tag=("d", "x"))))
        self.assertEqual([], list(d.iterancestors("d", "x")))

        self.assertEqual([b, a], list(d.iterancestors(tag=("b", "*"))))
        self.assertEqual([b, a], list(d.iterancestors("b", "*")))

        self.assertEqual([b], list(d.iterancestors(tag=("b", "c"))))
        self.assertEqual([b], list(d.iterancestors("b", "c")))
    ...
    def test_iterdescendants(self):
        ...
        self.assertEqual([b, d, c, e], list(a.iterdescendants()))
        self.assertEqual([], list(d.iterdescendants()))
    ...
    def test_iterdescendants_tag(self):
        ...
        self.assertEqual([], list(a.iterdescendants("a")))
        self.assertEqual([], list(a.iterdescendants(tag="a")))

        a2 = SubElement(e, "a")
        self.assertEqual([a2], list(a.iterdescendants("a")))

        self.assertEqual([a2], list(c.iterdescendants("a")))
        self.assertEqual([a2], list(c.iterdescendants(tag="a")))
    ...
    def test_iterdescendants_tag_multiple(self):
        ...
        self.assertEqual([b, e], list(a.iterdescendants(tag=("a", "b", "e"))))
        self.assertEqual([b, e], list(a.iterdescendants("a", "b", "e")))

        a2 = SubElement(e, "a")
        self.assertEqual([b, a2], list(a.iterdescendants(tag=("a", "b"))))
        self.assertEqual([b, a2], list(a.iterdescendants("a", "b")))

        self.assertEqual([], list(c.iterdescendants(tag=("x", "y", "z"))))
        self.assertEqual([], list(c.iterdescendants("x", "y", "z")))

        self.assertEqual(
            [b, d, c, e, a2], list(a.iterdescendants(tag=("x", "y", "z", "*")))
        )
        self.assertEqual(
            [b, d, c, e, a2], list(a.iterdescendants("x", "y", "z", "*"))
        )
    ...
    def test_getroottree(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(b, "d")
        self.assertEqual(a, a.getroottree().getroot())
        self.assertEqual(a, b.getroottree().getroot())
        self.assertEqual(a, d.getroottree().getroot())
    ...
    def test_getnext(self):
        ...
        b = SubElement(a, "b")
        c = SubElement(a, "c")
        self.assertEqual(None, a.getnext())
        self.assertEqual(c, b.getnext())
        self.assertEqual(None, c.getnext())
    ...
    def test_getprevious(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(b, "d")
        self.assertEqual(None, a.getprevious())
        self.assertEqual(b, c.getprevious())
        self.assertEqual(None, b.getprevious())
    ...
    def test_itersiblings(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(b, "d")
        self.assertEqual([], list(a.itersiblings()))
        self.assertEqual([c], list(b.itersiblings()))
        self.assertEqual([], list(c.itersiblings()))
        self.assertEqual([b], list(c.itersiblings(preceding=True)))
        self.assertEqual([], list(b.itersiblings(preceding=True)))
    ...
    def test_itersiblings_tag(self):
        ...
        c = SubElement(a, "c")
        d = SubElement(b, "d")
        self.assertEqual([], list(a.itersiblings(tag="XXX")))
        self.assertEqual([c], list(b.itersiblings(tag="c")))
        self.assertEqual([c], list(b.itersiblings(tag="*")))
        self.assertEqual([b], list(c.itersiblings(preceding=True, tag="b")))
        self.assertEqual([], list(c.itersiblings(preceding=True, tag="c")))
    ...
    def test_itersiblings_tag_multiple(self):
        ...
        d = SubElement(b, "d")
        e = SubElement(a, "e")
        self.assertEqual([], list(a.itersiblings(tag=("XXX", "YYY"))))
        self.assertEqual([c, e], list(b.itersiblings(tag=("c", "d", "e"))))
        self.assertEqual(
    ...
    def test_parseid(self):
        ...
        root = tree.getroot()
        root2 = XML(xml_text)
        self.assertEqual(self._writeElement(root), self._writeElement(root2))
        expected = {"chapter1": root[0], "xmlid": root[3], "warn1": root[4]}
    ...
    def _checkIDDict(self, dic, expected):
        self.assertEqual(len(dic), len(expected))
        self.assertEqual(sorted(dic.items()), sorted(expected.items()))
        if sys.version_info < (3,):
            self.assertEqual(
                sorted(dic.iteritems()), sorted(expected.iteritems())
            )
        self.assertEqual(sorted(dic.keys()), sorted(expected.keys()))
        if sys.version_info < (3,):
            self.assertEqual(
                sorted(dic.iterkeys()), sorted(expected.iterkeys())
            )
        if sys.version_info < (3,):
            self.assertEqual(sorted(dic.values()), sorted(expected.values()))
            self.assertEqual(
                sorted(dic.itervalues()), sorted(expected.itervalues())
            )

    def test_register_namespace_xml(self):
        self.assertRaises(
            ValueError,
            self.etree.register_namespace,
            "XML",
    ...
    def test_namespaces_default_and_other(self):
        ...
        self.assertEqual(
            _bytes(
                '<bar xmlns="http://ns.infrae.com/foo" xmlns:p="http://test/"></bar>'
            ),
            self._writeElement(e),
        )
    ...
    def test_attribute_keeps_namespace_prefix_on_merge(self):
        ...
        root = etree.Element(
            "{http://test/ns}root", nsmap={None: "http://test/ns"}
        )
        sub = etree.Element(
            "{http://test/ns}sub", nsmap={"test": "http://test/ns"}
        )
    ...
    def test_attribute_gets_namespace_prefix_on_merge_with_nons(self):
        ...
        root = etree.Element("root")
        sub = etree.Element(
            "{http://test/ns}sub", nsmap={None: "http://test/ns"}
        )
    ...
    def test_attribute_gets_namespace_prefix_on_merge(self):
        ...
        root = etree.Element(
            "{http://test/ns}root",
            nsmap={"test": "http://test/ns", None: "http://test/ns"},
        )
        sub = etree.Element(
            "{http://test/ns}sub", nsmap={None: "http://test/ns"}
        )
        ...
        self.assertEqual(
            _bytes(
                '<test:root xmlns:test="http://test/ns" xmlns="http://test/ns">'
    ...
    def test_namespaces_default_copy_element(self):
        ...
        self.assertEqual(None, e1.prefix)
        self.assertEqual(None, e1[0].prefix)
        self.assertEqual("{http://ns.infrae.com/foo}bar", e1.tag)
        self.assertEqual("{http://ns.infrae.com/foo}bar", e1[0].tag)
    ...
    def test_namespaces_copy_element(self):
        ...
        self.assertEqual(None, e1.prefix)
        self.assertNotEqual(None, e2.prefix)
        self.assertEqual("{http://ns.infrae.com/BAR}bar", e1.tag)
        self.assertEqual("{http://ns.infrae.com/foo}bar", e2.tag)
    ...
    def test_namespaces_reuse_after_move(self):
        ...
        two = self.etree.fromstring(_bytes('<root xmlns:ns="%s"/>' % ns_href))
        two.append(baz)
    ...
    def test_namespace_cleanup(self):
        ...
        self.assertEqual(
            _bytes(
                '<foo xmlns="F"><bar xmlns:ns="NS" xmlns="B"><ns:baz/></bar></foo>'
            ),
            self.etree.tostring(root),
        )
    ...
    def test_namespace_cleanup_deep_to_top(self):
        ...
        self.assertEqual(
            b'<root xmlns:n64="NS64">'
            + b"<a>" * 100
            + b'<n64:x xmlns:a="A" a:attr="X"/>'
            + b"</a>" * 100
            + b"</root>",
            self.etree.tostring(root),
        )
    ...
    def test_namespace_cleanup_keep_prefixes(self):
        xml = (
            '<root xmlns:n64="NS64" xmlns:foo="FOO" xmlns:unused1="UNUSED" xmlns:no="NO">'
    ...
    def test_namespace_cleanup_keep_prefixes_top(self):
        xml = (
            '<root xmlns:n64="NS64" xmlns:unused1="UNUSED" xmlns:no="NO">'
            '<sub xmlns:foo="FOO">'
            '<a xmlns:unused2="UNUSED"><n64:x xmlns:a="A" a:attr="X"/></a>'
            "<foo>foo:bar</foo>"
            "</sub>"
    ...
    def test_html_prefix_nsmap(self):
        etree = self.etree
        el = etree.HTML(
            "<hha:page-description>aa</hha:page-description>"
    ...
    def test_getiterator_filter_multiple(self):
        ...
        self.assertEqual([a, b], list(a.getiterator("a", "b")))
        self.assertEqual([], list(a.getiterator("x", "y")))
        self.assertEqual([a, f], list(a.getiterator("f", "a")))
        self.assertEqual([c, e, f], list(c.getiterator("c", "*", "a")))
        self.assertEqual([], list(a.getiterator((), ())))
    ...
    def test_getiterator_filter_namespace(self):
        ...
        self.assertEqual([a], list(a.getiterator("{a}a")))
        self.assertEqual([], list(a.getiterator("{b}a")))
        self.assertEqual([], list(a.getiterator("a")))
        self.assertEqual([a, b, d, c, e, f, g], list(a.getiterator("*")))
        self.assertEqual([f], list(c.getiterator("{b}*")))
        self.assertEqual([d, f], list(a.getiterator("{b}*")))
        self.assertEqual([g], list(a.getiterator("g")))
        self.assertEqual([g], list(a.getiterator("{}g")))
        self.assertEqual([g], list(a.getiterator("{}*")))
    ...
    def test_getiterator_filter_local_name(self):
        ...
        self.assertEqual([b, c, d], list(a.getiterator("{*}b")))
        self.assertEqual([e, f, g], list(a.getiterator("{*}e")))
        self.assertEqual([a, b, c, d, e, f, g], list(a.getiterator("{*}*")))
    ...
    def test_getiterator_filter_entities(self):
        ...
        self.assertEqual([entity_b], list(a.getiterator(Entity)))

        entity_a = Entity("TEST-a")
        a.append(entity_a)

        self.assertEqual([entity_b, entity_a], list(a.getiterator(Entity)))

        self.assertEqual([entity_b], list(b.getiterator(Entity)))
    ...
    def test_getiterator_filter_element(self):
        ...
        self.assertEqual([a, b, c], list(a.getiterator(Element)))
    ...
    def test_getiterator_filter_all_comment_pi(self):
        ...
        self.assertEqual([a, b, c], list(a.getiterator("*")))
    ...
    def test_elementtree_getelementpath_ns(self):
        ...
        self.assertEqual(".", tree.getelementpath(a))
        self.assertEqual(
            "{http://ns1/}c/{http://ns1/}d[1]", tree.getelementpath(d1)
        )
        self.assertEqual(
            "{http://ns1/}c/{http://ns2/}d", tree.getelementpath(d2)
        )
        self.assertEqual(
            "{http://ns1/}c/{http://ns1/}d[2]", tree.getelementpath(d3)
        )
    ...
    def test_elementtree_iter_qname(self):
        ...
        self.assertEqual(
            list(tree.iter(QName("b"))), list(tree.iter("b")),
        )
        self.assertEqual(
            list(tree.iter(QName("X", "b"))), list(tree.iter("{X}b")),
        )
    ...
    def test_findall_syntax_error(self):
        ...
        self.assertRaises(
            SyntaxError, root.findall, "//"
        )  # absolute path on Element
        self.assertRaises(SyntaxError, root.findall, ".///")
    ...
    def test_index(self):
        ...
        for i in range(10):
            self.assertEqual(i, e.index(e[i]))
        self.assertEqual(3, e.index(e[3], 3))
        self.assertRaises(ValueError, e.index, e[3], 4)
        self.assertRaises(ValueError, e.index, e[3], 0, 2)
        self.assertRaises(ValueError, e.index, e[8], 0, -3)
        self.assertRaises(ValueError, e.index, e[8], -5, -3)
        self.assertEqual(8, e.index(e[8], 0, -1))
        self.assertEqual(8, e.index(e[8], -12, -1))
        self.assertEqual(0, e.index(e[0], -12, -1))
    ...
    def test_replace(self):
        ...
        self.assertEqual(child1, e[0])
        self.assertEqual(child1.text, "text1")
        self.assertEqual(child1.tail, "tail1")
        self.assertEqual(child0.tail, "tail0")
        self.assertEqual(child2, e[1])

        e.replace(e[-1], e[0])
        self.assertEqual(child1, e[-1])
        self.assertEqual(child1.text, "text1")
        self.assertEqual(child1.tail, "tail1")
        self.assertEqual(child2, e[0])
    ...
    def test_replace_new(self):
        ...
        self.assertEqual(new_element, e[0])
        self.assertEqual("TESTTEXT", e[0].text)
        self.assertEqual("TESTTAIL", e[0].tail)
        self.assertEqual(child1, e[1])
    ...
    def test_setslice_all_reversed(self):
        ...
        a[:] = [e, f, g]
        self.assertEqual([e, f, g], list(a))

        a[::-1] = [e, f, g]
        self.assertEqual([g, f, e], list(a))
    ...
    def test_setslice_step(self):
        ...
        a[1::2] = [x, y]
        self.assertEqual([b, x, d, y], list(a))
    ...
    def test_setslice_step_negative(self):
        ...
        a[1::-1] = [x, y]
        self.assertEqual([y, x, d, e], list(a))
    ...
    def test_setslice_step_negative2(self):
        ...
        a[::-2] = [x, y]
        self.assertEqual([b, y, d, x], list(a))
    ...
    def test_setslice_step_overrun(self):
        ...
        self.assertRaises(
            ValueError, operator.setitem, a, slice(1, None, 2), [x, y, z]
        )

        self.assertEqual([b, c, d, e], list(a))
    ...
    def test_sourceline_XML(self):
        ...
        self.assertEqual(
            [2, 2, 4], [el.sourceline for el in root.getiterator()]
        )
    ...
    def test_sourceline_parse(self):
        ...
        self.assertEqual(
            [1, 2, 3], [el.sourceline for el in tree.getiterator()]
        )
    ...
    def test_sourceline_iterparse_end(self):
        ...
        lines = [
            el.sourceline
            for (event, el) in iterparse(
                fileInTestDir("include/test_xinclude.xml")
            )
        ]

        self.assertEqual([2, 3, 1], lines)
    ...
    def test_sourceline_iterparse_start(self):
        ...
        lines = [
            el.sourceline
            for (event, el) in iterparse(
                fileInTestDir("include/test_xinclude.xml"), events=("start",)
            )
        ]
    ...
    def test_docinfo_public(self):
        ...
        xml = _bytes(
            xml_header + doctype_string + "<html><body></body></html>"
        )
    ...
    def test_doctype_output_override(self):
        ...
        pub_id = "-//W3C//DTD XHTML 1.0 Transitional//EN"
        sys_id = "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"
        doctype_string = _bytes(
            '<!DOCTYPE html PUBLIC "%s" "%s">' % (pub_id, sys_id)
        )
    ...
    def test_html_base(self):
        etree = self.etree
        root = etree.HTML(
            _bytes("<html><body></body></html>"), base_url="http://no/such/url"
        )
        self.assertEqual(root.base, "http://no/such/url")
    ...
    def test_html_base_tag(self):
        ...
        root = etree.HTML(
            _bytes(
                '<html><head><base href="http://no/such/url"></head></html>'
            )
        )
        self.assertEqual(root.base, "http://no/such/url")
    ...
    def test_indent(self):
        ...
        elem = ET.XML(
            "<html><body><p>pre<br/>post</p><p>text</p></body></html>"
        )
        ET.indent(elem)
        self.assertEqual(
            ET.tostring(elem),
    ...
    def test_dtd_io(self):
        ...
        self.assertEqual(
            self.etree.tostring(tree).replace(_bytes(" "), _bytes("")),
    ...
    def test_unicode_byte_zero(self):
        ...
        a = Element("a")
        self.assertRaises(ValueError, setattr, a, "text", _str("ha\0ho"))
        self.assertRaises(ValueError, setattr, a, "tail", _str("ha\0ho"))

        self.assertRaises(ValueError, Element, _str("ha\0ho"))
    ...
    def test_unicode_byte_invalid(self):
        ...
        a = Element("a")
        self.assertRaises(ValueError, setattr, a, "text", _str("ha\x07ho"))
        self.assertRaises(ValueError, setattr, a, "text", _str("ha\x02ho"))

        self.assertRaises(ValueError, setattr, a, "tail", _str("ha\x07ho"))
        self.assertRaises(ValueError, setattr, a, "tail", _str("ha\x02ho"))

        self.assertRaises(ValueError, Element, _str("ha\x07ho"))
    ...
    def test_unicode_byte_invalid_sequence(self):
        ...
        a = Element("a")
        self.assertRaises(
            ValueError, setattr, a, "text", _str("ha\u1234\x07ho")
        )
        self.assertRaises(
            ValueError, setattr, a, "text", _str("ha\u1234\x02ho")
        )

        self.assertRaises(
            ValueError, setattr, a, "tail", _str("ha\u1234\x07ho")
        )
        self.assertRaises(
            ValueError, setattr, a, "tail", _str("ha\u1234\x02ho")
        )

        self.assertRaises(ValueError, Element, _str("ha\u1234\x07ho"))
        self.assertRaises(ValueError, Element, _str("ha\u1234\x02ho"))
    ...
    def test_encoding_tostring_utf16(self):
        ...
        self.assertEqual(_bytes("<a><b></b><c></c></a>"), canonicalize(result))
    ...
    def test_tostring_method_html_with_tail(self):
        ...
        self.assertEqual(
            result, _bytes("<div><p>Some text<i>\r\n</i></p></div>\r\n")
        )
    ...
    def test_tostring_standalone(self):
        ...
        result = tostring(
            root, xml_declaration=True, encoding="ASCII", standalone=True
        )
        self.assertEqual(
            result,
            _bytes(
        ...
        result = tostring(
            root, xml_declaration=True, encoding="ASCII", standalone=False
        )
        self.assertEqual(
            result,
            _bytes(
    ...
    def test_tostring_method_text_encoding(self):
        ...
        self.assertEqual(_str("ABSøk på nettetCtail").encode("UTF-16"), result)
    ...
    def test_tostring_method_text_unicode(self):
        ...
        self.assertRaises(UnicodeEncodeError, tostring, a, method="text")
    ...
    def test_tounicode(self):
        ...
        self.assertEqual(
            _bytes("<a><b></b><c></c></a>"), canonicalize(tounicode(a))
        )
    ...
    def test_tounicode_element(self):
        ...
        self.assertEqual(_bytes("<b></b>"), canonicalize(tounicode(b)))
        self.assertEqual(_bytes("<c><d></d></c>"), canonicalize(tounicode(c)))
    ...
    def test_tounicode_element_tail(self):
        ...
        self.assertTrue(isinstance(tounicode(b), _unicode))
        self.assertTrue(
            tounicode(b) == "<b/>Foo" or tounicode(b) == "<b />Foo"
        )
    ...
    def test_tostring_unicode_element(self):
        ...
        self.assertEqual(
            _bytes("<b></b>"), canonicalize(tostring(b, encoding=_unicode))
        )
        self.assertEqual(
            _bytes("<c><d></d></c>"),
    ...
    def test_tostring_unicode_none(self):
        tostring = self.etree.tostring
        self.assertRaises(
            TypeError, self.etree.tostring, None, encoding=_unicode
        )
    ...
    def test_tostring_unicode_element_tail(self):
        ...
        self.assertTrue(
            ...
            or tostring(b, encoding=_unicode) == "<b />Foo"
        )
...
class _XIncludeTestCase(HelperTestCase):
    def test_xinclude_text(self):
        ...
        self.assertEqual(old_text + content + old_tail, root.text)

    def test_xinclude(self):
        tree = etree.parse(fileInTestDir("include/test_xinclude.xml"))
        self.assertNotEqual("a", tree.getroot()[1].tag)
        # process xincludes
        self.include(tree)
        # check whether we find it replaced with included data
        self.assertEqual("a", tree.getroot()[1].tag)
    ...
    def test_xinclude_resolver(self):
        ...
        tree = etree.parse(
            fileInTestDir("include/test_xinclude.xml"), parser=parser
        )
        ...
        called.sort()
        self.assertEqual(
            [("dtd", True), ("include", True), ("input", True)], called
        )
...
class ElementIncludeTestCase(_XIncludeTestCase):
    ...
    def include(self, tree, loader=None, max_depth=None):
        self.ElementInclude.include(
            tree.getroot(), loader=loader, max_depth=max_depth
        )
    ...
    def test_xinclude_failures(self):
        ...
        self.assertEqual(
            str(cm.exception), "recursive include of 'Recursive2.xml' detected"
        )
        ...
        with self.assertRaises(
            self.ElementInclude.LimitedRecursiveIncludeError
        ) as cm:
...
class ETreeC14NTestCase(HelperTestCase):
    def test_c14n(self):
        ...
        tree.write_c14n(f)
        s = f.getvalue()
        self.assertEqual(_bytes("<a><b></b></a>"), s)
    ...
    def test_c14n_gzip(self):
        ...
        with gzip.GzipFile(fileobj=BytesIO(f.getvalue())) as gzfile:
            s = gzfile.read()
        self.assertEqual(_bytes("<a>" + "<b></b>" * 200 + "</a>"), s)
    ...
    def test_c14n_file(self):
        ...
        self.assertEqual(_bytes("<a><b></b></a>"), data)
    ...
    def test_c14n_file_gzip(self):
        ...
        with tmpfile() as filename:
            ...
            with gzip.open(filename, "rb") as f:
                data = f.read()
        self.assertEqual(_bytes("<a>" + "<b></b>" * 200 + "</a>"), data)
    ...
    def test_c14n2_with_text(self):
        ...
        s = f.getvalue()
        self.assertEqual(
            b"<a> abc \n <b>  btext </b> btail <c></c>    ctail </a>", s
        )

        f = BytesIO()
        tree.write(f, method="c14n2", strip_text=True)
        s = f.getvalue()
        self.assertEqual(b"<a>abc<b>btext</b>btail<c></c>ctail</a>", s)
    ...
    def test_c14n_exclusive(self):
        ...
        self.assertEqual(
            _bytes(
                '<a xmlns="http://abc" xmlns:y="http://bcd" xmlns:z="http://cde"><z:b></z:b></a>'
            ),
            s,
        )
        f = BytesIO()
        ...
        self.assertEqual(
            _bytes(
                '<a xmlns="http://abc"><z:b xmlns:z="http://cde"></z:b></a>'
            ),
            s,
        )
        ...
        self.assertEqual(
            _bytes(
                '<a xmlns="http://abc" xmlns:z="http://cde"><z:b></z:b></a>'
            ),
            s,
        )
    ...
    def test_c14n_tostring_exclusive(self):
        ...
        s = etree.tostring(
            tree, method="c14n", exclusive=True, inclusive_ns_prefixes=["y"]
        )
        self.assertEqual(
            _bytes(
                '<a xmlns="http://abc" xmlns:y="http://bcd"><z:b xmlns:z="http://cde"></z:b></a>'
            ),
            s,
        )
    ...
    def test_c14n_element_tostring_exclusive(self):
        ...
        self.assertEqual(
            _bytes(
                '<z:b xmlns="http://abc" xmlns:y="http://bcd" xmlns:z="http://cde"></z:b>'
            ),
            s,
        )
        s = etree.tostring(tree.getroot()[0], method="c14n", exclusive=True)
        self.assertEqual(_bytes('<z:b xmlns:z="http://cde"></z:b>'), s)

        s = etree.tostring(
            tree.getroot()[0],
            method="c14n",
            exclusive=True,
            inclusive_ns_prefixes=["y"],
        )
        self.assertEqual(
            _bytes('<z:b xmlns:y="http://bcd" xmlns:z="http://cde"></z:b>'), s
        )
    ...
    def test_c14n_tostring_inclusive_ns_prefixes(self):
        ...
        s = etree.tostring(
            ...
            method="c14n",
            exclusive=True,
            inclusive_ns_prefixes=["x", "y", "z"],
        )
        self.assertEqual(
            _bytes(
                '<a xmlns:x="http://abc" xmlns:y="http://bcd" xmlns:z="http://cde"><z:b></z:b></a>'
            ),
            s,
        )
...
class ETreeWriteTestCase(HelperTestCase):
    def test_write(self):
        ...
        tree.write(f)
        s = f.getvalue()
        self.assertEqual(_bytes("<a><b/></a>"), s)
    ...
    def test_write_gzip(self):
        ...
        with gzip.GzipFile(fileobj=BytesIO(f.getvalue())) as gzfile:
            s = gzfile.read()
        self.assertEqual(_bytes("<a>" + "<b/>" * 200 + "</a>"), s)
    ...
    def test_write_gzip_level(self):
        ...
        self.assertEqual(_bytes("<a>" + "<b/>" * 200 + "</a>"), s0)
        self.assertEqual(_bytes("<a>" + "<b/>" * 200 + "</a>"), s1)
        self.assertEqual(_bytes("<a>" + "<b/>" * 200 + "</a>"), s9)
    ...
    def test_write_file(self):
        ...
        with tmpfile() as filename:
            tree.write(filename)
            data = read_file(filename, "rb")
        self.assertEqual(_bytes("<a><b/></a>"), data)
    ...
    def test_write_file_gzip(self):
        ...
        with tmpfile() as filename:
            ...
            with gzip.open(filename, "rb") as f:
                data = f.read()
        self.assertEqual(_bytes("<a>" + "<b/>" * 200 + "</a>"), data)
...
class ETreeErrorLogTest(HelperTestCase):
    ...
    def test_parse_error_logging(self):
        ...
        try:
            ...
            logs = e.error_log
        f.close()
        self.assertTrue([log for log in logs if "mismatch" in log.message])
        self.assertTrue([log for log in logs if "PARSER" in log.domain_name])
        self.assertTrue(
            [log for log in logs if "ERR_TAG_NAME_MISMATCH" in log.type_name]
        )
        self.assertTrue([log for log in logs if 1 == log.line])
        self.assertTrue([log for log in logs if 15 == log.column])
    ...
    def _test_python_error_logging(self):
        ...
        self.assertTrue(
            [message for message in messages if "mismatch" in message]
        )
        self.assertTrue(
            [message for message in messages if ":PARSER:" in message]
        )
        self.assertTrue(
            [
                message
        ...
        self.assertTrue(
            [message for message in messages if ":1:15:" in message]
        )
...
class XMLPullParserTest(unittest.TestCase):
    ...
    def assert_event_tags(self, events, expected):
        self.assertEqual(
            [(action, elem.tag) for action, elem in events], expected
        )
    ...
    def test_pull_from_simple_target_start_end(self):
        ...
        parser = self.etree.XMLPullParser(["start", "end"], target=Target())
        events = parser.read_events()
        ...
        parser.feed("</element><child>")
        self.assertEqual(
            [("end", "end(element)"), ("start", "start(child)")], list(events)
        )
        parser.feed("</child>")
        self.assertEqual([("end", "end(child)")], list(events))
        parser.feed("</root>")
    ...
    def test_pull_from_tree_builder(self):
        ...
        parser.feed("</child>")
        self.assert_event_tags(events, [("end", "child")])
        parser.feed("</root>")
        self.assert_event_tags(events, [("end", "root")])
        self.assertFalse(list(events))
        root = parser.close()
    ...
    def test_pull_from_tree_builder_subclass(self):
        ...
        parser.feed("</child>")
        self.assert_event_tags(events, [("end", "child-huhu")])
        parser.feed("</root>")
        self.assert_event_tags(events, [("end", "root-huhu")])
        self.assertFalse(list(events))
        root = parser.close()
...
def test_suite():
    ...
    suite.addTests([make_doctest("../../../doc/tutorial.txt")])
    suite.addTests([make_doctest("../../../doc/api.txt")])
    suite.addTests([make_doctest("../../../doc/FAQ.txt")])
    suite.addTests([make_doctest("../../../doc/parsing.txt")])
    suite.addTests([make_doctest("../../../doc/resolvers.txt")])
    return suite
