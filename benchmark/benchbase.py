...
class TreeBenchMark(object):
    ...
    def _setup_tree3(self, text, attributes):
        ...
        for i in range(6 + TREE_FACTOR):
            children = [
                SubElement(c, "{cdefg}a%05d" % (i % 8), attributes)
...
def buildSuites(benchmark_class, etrees, selected):
    ...
    if selected:
        benchmarks = [
            [b for b in bs if [match for match in selected if match(b[0])]]
...
def printSetupTimes(benchmark_suites):
    ...
    for b in benchmark_suites:
        ...
        for i, tree_times in enumerate(b.setup_times):
            print(
                "     T%d: %s"
                % (i + 1, " ".join("%6.4f" % t for t in tree_times))
            )
    print("")
...
def runBench(
    ...
    tree_builders = [
        suite.tree_builder(tree, tn, an, serial, children) for tree in tree_set
    ]
...
def runBenchmarks(benchmark_suites, benchmarks):
    for bench_calls in izip(*benchmarks):
        for lib, (bench, benchmark_setup) in enumerate(
            izip(benchmark_suites, bench_calls)
        ):
...
def main(benchmark_class):
    ...
    benchmark_suites, benchmarks = buildSuites(
        benchmark_class, _etrees, selected
    )

    print(
        "Running benchmark on", ", ".join(b.lib_name for b in benchmark_suites)
    )
    print("")
