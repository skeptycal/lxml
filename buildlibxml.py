...
def download_and_extract_windows_binaries(destdir):
    ...
    release_path = "/download/%s/" % find_max_version(
        "library release",
        filenames,
        re.compile(r"/releases/tag/([0-9.]+[0-9])$"),
    )
    url += release_path
    filenames = [
        filename.rsplit("/", 1)[1]
...
def remote_listdir(url):
    try:
        ...
        print(
            "Requesting with urllib failed. Falling back to ftplib. "
...
def download_zlib(dest_dir, version):
    ...
    return download_library(
        dest_dir, ZLIB_LOCATION, "zlib", version_re, filename, version=version
    )
...
def download_library(
    dest_dir, location, name, version_re, filename, version=None
):
    if version is None:
        try:
            ...
            for fn in fns:
                if fn.startswith(name + "-"):
                    ...
                    if match:
                        version_tuple = tuple(
                            map(tryint, match.group(1).split("."))
                        )
    ...
    if os.path.exists(dest_filename):
        ...
        print(
            "Downloading %s into %s from %s" % (name, dest_filename, full_url)
        )
        urlcleanup()  # work around FTP bug 27973 in Py2.7.12
...
def call_subprocess(cmd, **kw):
    ...
    if returncode:
        raise Exception(
            'Command "%s" returned code %s' % (cmd_desc, returncode)
        )
...
def cmmi(configure_cmd, build_dir, multicore=None, **call_setup):
    ...
    if not multicore:
        ...
        make_jobs = []
    call_subprocess(["make"] + make_jobs, cwd=build_dir, **call_setup)
    call_subprocess(
        ["make"] + make_jobs + ["install"], cwd=build_dir, **call_setup
    )
...
def configure_darwin_env(env_setup):
    ...
    major_version, minor_version = tuple(
...
def build_libxml2xslt(
    ...
    zlib_dir = unpack_tarball(
        download_zlib(download_dir, zlib_version), build_dir
    )
    libiconv_dir = unpack_tarball(
        download_libiconv(download_dir, libiconv_version), build_dir
    )
    libxml2_dir = unpack_tarball(
        download_libxml2(download_dir, libxml2_version), build_dir
    )
    libxslt_dir = unpack_tarball(
        download_libxslt(download_dir, libxslt_version), build_dir
    )
    ...
    if tuple(map(tryint, libxml2_version.split("-", 1)[0].split("."))) >= (
        2,
        9,
        5,
    ):
        libxml2_configure_cmd.append(
            "--without-lzma"
        )  # can't currently build that
